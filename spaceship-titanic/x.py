...
# Box plot
plt.subplot(1, 2, 1)
sns.boxplot(x=cat_feat[0], y=num_feat[0], data=combined_df, palette="Set3", hue='Transported')
plt.title(f'Box plot of {num_feat[0]} by {cat_feat[0]}')

# Swarm plot
plt.subplot(1, 2, 2)
sns.swarmplot(x=cat_feat[0], y=num_feat[0], data=combined_df, palette="Set3", alpha=0.5, hue='Transported')
